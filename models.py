...
import subprocess
import json
from ember import PEFeatureExtractor, lgb
...
class MalConvModel(object):
    ...
    def is_evasive_score(self, file_path):
        score = self.get_score(file_path)
        #print(os.path.basename(file_path), score)
        return score < self.thresh, score
...
class EmberModel_2019(object):       # model in MLSEC 2019
    ...
    def get_score(self,feature):
        ...
        score = self.predict_sample(self.model, feature)
        return score
    
    def is_evasive(self, feature):
    ...
    def predict_sample(self, lgbm_model, feature, feature_version=2):
        extractor = PEFeatureExtractor(2)
        a = extractor.process_raw_features(feature)
        
        features = np.array(a, dtype=np.float32)
        return lgbm_model.predict([features])[0]

    # def test(lgbm_model, file_data, feature_version=2):
    #     """
    #     Predict a PE file with an LightGBM model
    #     """
    #     extractor = PEFeatureExtractor(feature_version)
    #     a = extractor.process_raw_features(self.raw_features(file_data))
    #     c = self.raw_features(file_data)
