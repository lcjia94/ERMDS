...
if __name__ == '__main__':
    obs_dir = r'D:\MAB\benign_POS1'
    json_file = r'D:\MAB\utils\benign_POS2.json'
    binarys = [f"{obs_dir}\{x}" for x in os.listdir(obs_dir)]

    data = []
    for binary_path in binarys:
        file_data = open(binary_path, "rb").read()
        

        extractor = PEFeatureExtractor(2)
        a = extractor.raw_features(file_data)
        a['obfuscation'] = 'POS'
        a['label'] = 0
        a = json.dumps(obj=a)
